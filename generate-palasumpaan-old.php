...
try {
    ...
    // For generate mode, convert to PDF
    // Save the processed template temporarily
    $tempDocx = tempnam(sys_get_temp_dir(), 'palasumpaan_') . '.docx';
    $templateProcessor->saveAs($tempDocx);
    
    // Configure PHPWord to use HTML writer
    Settings::setOutputEscapingEnabled(true);
    
    // Load the saved document and convert to HTML
    $phpWord = \PhpOffice\PhpWord\IOFactory::load($tempDocx);
    $htmlWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'HTML');
    
    // Save HTML to temporary file
    $tempHtml = tempnam(sys_get_temp_dir(), 'palasumpaan_') . '.html';
    ...
    $dompdf->stream($fileName, ['Attachment' => false]); // false = display in browser, true = download
