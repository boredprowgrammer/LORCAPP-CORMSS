...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <h2 class="text-2xl font-semibold text-gray-900">Transfer Out Officer</h2>
        </div>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div class="flex items-start">
                ...
                <div>
                    <p class="font-semibold text-yellow-800">Transfer Out Process</p>
                    <p class="text-sm text-yellow-700 mt-1">This will mark the officer as inactive and decrease headcount by -1. Week number will be auto-generated based on transfer date.</p>
                </div>
            </div>
        </div>
        ...
        <form method="POST" action="" class="space-y-6" x-data="{ officerSelected: false, selectedOfficer: null }">
            ...
            <div x-show="officerSelected" class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <span class="text-sm font-medium text-blue-800">Officer selected. Continue with transfer details below.</span>
                </div>
            </div>
            
            <!-- Destination Information -->
            <div class="border-b border-gray-200 pb-2 mb-6">
                <h3 class="text-lg font-semibold text-gray-900">Destination (Where officer is transferring to)</h3>
            </div>
        ...
        fetch('<?php echo BASE_URL; ?>/api/search-officers.php?q=' + encodeURIComponent(query))
            ...
            .then(data => {
                ...
                data.forEach(officer => {
                    html += `
                        <div class="p-3 hover:bg-base-300 cursor-pointer" onclick="selectOfficer('${officer.id}', '${officer.name.replace(/'/g, "\\'")}', '${officer.full_name.replace(/'/g, "\\'")}', '${officer.location.replace(/'/g, "\\'")}')">
